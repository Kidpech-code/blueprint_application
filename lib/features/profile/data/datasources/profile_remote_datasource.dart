...
abstract class ProfileRemoteDataSource {
  ...
  Future<ProfileModel> updateProfile(String userId, UpdateProfileRequest request);
...
class ProfileRemoteDataSourceImpl implements ProfileRemoteDataSource {
  ...
  @override
  Future<ProfileModel> updateProfile(String userId, UpdateProfileRequest request) async {
    try {
      final response = await dio.put('/profiles/$userId', data: request.toJson());

      if (response.statusCode == 200) {
        return ProfileModel.fromJson(response.data);
      } else {
        throw ServerError('Failed to update profile', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  Future<ProfileStatsModel> getProfileStats(String userId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw ServerError('Failed to get profile stats', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  Future<String> uploadProfileImage(String userId, String imagePath) async {
    try {
      final formData = FormData.fromMap({'image': await MultipartFile.fromFile(imagePath)});

      final response = await dio.post('/profiles/$userId/profile-image', data: formData);

      if (response.statusCode == 200) {
        return response.data['image_url'] as String;
      } else {
        throw ServerError('Failed to upload profile image', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  Future<String> uploadCoverImage(String userId, String imagePath) async {
    try {
      ...
      final response = await dio.post('/profiles/$userId/cover-image', data: formData);

      if (response.statusCode == 200) {
        return response.data['image_url'] as String;
      } else {
        throw ServerError('Failed to upload cover image', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  Future<void> followUser(String userId, String accessToken) async {
    try {
      final response = await dio.post('/profiles/$userId/follow', options: Options(headers: {'Authorization': 'Bearer $accessToken'}));
  ...
  Future<void> unfollowUser(String userId, String accessToken) async {
    try {
      final response = await dio.delete('/profiles/$userId/follow', options: Options(headers: {'Authorization': 'Bearer $accessToken'}));

      if (response.statusCode != 200) {
        throw ServerError('Failed to unfollow user', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  Future<bool> isFollowing(String userId, String accessToken) async {
    try {
      final response = await dio.get('/profiles/$userId/is-following', options: Options(headers: {'Authorization': 'Bearer $accessToken'}));

      if (response.statusCode == 200) {
        return response.data['is_following'] as bool;
      } else {
        throw ServerError('Failed to check follow status', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  @override
  Future<List<ProfileModel>> getFollowers(String userId, int page, int limit) async {
    try {
      final response = await dio.get('/profiles/$userId/followers', queryParameters: {'page': page, 'limit': limit});

      if (response.statusCode == 200) {
        final List<dynamic> data = response.data['data'] as List<dynamic>;
        return data.map((json) => ProfileModel.fromJson(json)).toList();
      } else {
        throw ServerError('Failed to get followers', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
  ...
  @override
  Future<List<ProfileModel>> getFollowing(String userId, int page, int limit) async {
    try {
      final response = await dio.get('/profiles/$userId/following', queryParameters: {'page': page, 'limit': limit});

      if (response.statusCode == 200) {
        final List<dynamic> data = response.data['data'] as List<dynamic>;
        return data.map((json) => ProfileModel.fromJson(json)).toList();
      } else {
        throw ServerError('Failed to get following', response.statusCode ?? 500);
      }
    } on DioException catch (e) {
      throw _handleDioError(e);
    } catch (e) {
